...
# 建议将其显示在屏幕上了解具体格式和数据内容）
print(pygal_countries)
gdp_countries = read_csv_as_nested_dict("isp_gdp.csv", "Country Code", ",", '"')
print(gdp_countries)


# for key, value in pygal_countries.items():
#     for key2, value2 in gdp_countries.items():
#         if value == value2['Country Name']:
#             print(key2, value2)

def reconcile_countries_by_name(plot_countries, gdp_country):  # 返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
    """
    输入参数:
    ...
    for key, value in plot_countries.items():
        list3.add(key)
        for key2, value2 in gdp_country.items():
            if value == value2['Country Name']:
                list1[key] = value2
                list2.add(key)
    list2 = list3-list2
...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
	...
	year: 具体年份值
    输出：
    输出包含一个字典和二个集合的元组数据。其中字典数据为绘图库各国家代码及对应的在某具体年份GDP产值（键为绘图库中各国家代码，值为在具体年份（由year参数确定）
    所对应的世行GDP数据值。为
   ...
   """
    gdp_country = read_csv_as_nested_dict(gdpinfo['gdpfile'], gdpinfo['country_code'], gdpinfo['separator'], gdpinfo['quote'])
    dicts_2 = set()
    dicts = reconcile_countries_by_name(plot_countries, gdp_country)
    dicts_1 = set(dicts[1])
    dicts_0 = dicts[0]
    yinfo={}
    'dicts_1 完全没有记录'
    'dicts_2 只是没有某特定年'
    for key, value in dicts_0.items():
        if value[year] == '':
            dicts_2.add(key)
        else:
            yinfo[key] = math.log(float(value[year]))

    t1 = (dicts_1, dicts_2, yinfo)
...
def render_world_map(gdpinfo, plot_countries, year, map_file):  # 将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    wm.title = "全球GDP分布图"
    atuple = build_map_dict_by_name(gdpinfo, plot_countries, year)
    dict_2 = atuple[0]
    dict_3 = atuple[1]
    dict_1 = atuple[2]
    wm.add(year, dict_1)
    ...
    print(dict_3)
    print(dict_1.keys())
    wm.add('missing from the world bank ', dict_2)
    wm.add('no data at this country', dict_3)
...
def test_render_world_map(year):  # 测试函数
    ...
    render_world_map(gdpinfo, pygal_countries, year, "isp_gdp_world_name_2001.svg")
